...
class Solution:
    def findAnagrams(self, s: str, p: str) -> List[int]:
        ...
        for i in range(window_len):
            pc = p[i]
            p_map[pc] =  p_map.get(pc, 0) + 1

        windowMap = {}
        for i in range(window_len):
            sc = s[i]
            windowMap[sc] =  windowMap.get(sc, 0) + 1
        
        for i in range(len(s) - window_len + 1):
            if windowMap == p_map:
                ans.append(i)
            if i == len(s) - window_len:
                break
            in_char = s[i+window_len]
            windowMap[in_char] = windowMap.get(in_char, 0) + 1
            out_char = s[i]
            if windowMap.get(out_char) == 1:
                windowMap.pop(out_char)
            elif out_char in windowMap:
                windowMap[out_char] = windowMap.get(out_char) - 1
        return ans
```
